...
// match[1]=kind  match[2]=text-block  match[3]=double-quoted (incl. "a" + "b" concatenation)
// match[4]=backtick  match[5]=string-array
...
export function parseJavaMapper(content: string): ParsedQuery[] {
  ...
  while ((match = JAVA_ANNOTATION_RE.exec(content)) !== null) {
    ...
    // match[2] = text block content (Java 15+), match[3] = quoted string (possibly
    // "a" + "b" + "c" concatenation), match[4] = backtick
    ...
    if (match[5] !== undefined) {
      ...
      // Join concatenated string literals ("a " + "b ") the way Java would — no separator added.
      const strings: string[] = [];
      const strRe = /"((?:[^"\\]|\\.)*)"/g;
      let sm: RegExpExecArray | null;
      while ((sm = strRe.exec(match[3])) !== null) { strings.push(sm[1]); }
      raw = strings.join('');
    } else {
      raw = match[2] ?? match[4] ?? '';
    }
